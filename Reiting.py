...
DEMO_DATA = {
    "ФИО": ["Учитель1", "Учитель2", "Учитель3"],
...
def parse_kl_ruk_sheet(df_raw):
    ...
    for idx in range(3, len(df_raw)):
        ...
        teachers.append({
            ...
            "Одарен дети": float(row[7]) if len(row) > 7 and pd.notna(row[7]) else 0,
...
display_columns = ['ФИО', 'Предмет', 'Коэф', 'Одарен дети итог', 'Метод итог', 'Админ итог', 'Жалпы итог']
...
st.dataframe(
    ...
    column_config={
        ...
        "Внекл итог": st.column_config.NumberColumn("Одарен дети итог", format="%.1f"),
